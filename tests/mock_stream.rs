...
async fn open_with_mock_returns_and_latest_updates() {
    let dev = open_with(OpenOptions::new(60.0).allow_mock(true)).await.expect("open mock");
    // latest should become Some within ~1s
    let mut found = false;
    for _ in 0..20 {
        if dev.latest().is_some() { found = true; break; }
...
async fn subscribe_yields_items_quickly() {
    ...
    let item = timeout(Duration::from_millis(500), s.next()).await.expect("no timeout");
    assert!(item.is_some(), "stream ended unexpectedly");
}

#[tokio::test(flavor = "current_thread")]
async fn smoothing_reduces_jitter() {
    let dev = open_with(OpenOptions::new(120.0).allow_mock(true)).await.expect("open mock");
    ...
    assert!(avg_smooth < avg_raw, "smoothing did not reduce jitter: smooth={} raw={}", avg_smooth, avg_raw);
}